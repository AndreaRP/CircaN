...
#' @export
#' @examples
#' circan_peak(data=expression_example, s2c=metadata_example, mode="port")
circan_peak <- function(data, s2c, shiny = FALSE, mode = "default", init_value = 24,
                              max_per = Inf, min_per = -Inf) {
  #############################
  # data <- subsampled_exprs_nls
  # s2c <- subsampled_s2c
  # data <- data_file[1:100,]
  # s2c <- meta_file
  # mode="port"
  # init_value = 24
  # max_per = Inf
  ...
  for (gene in 1:nrow(data)) {
    ...
    result = tryCatch({
      nls.model = nls(data ~ amp * (-1 + 2 * abs(cos(pi/per * time - phase*(pi/per)))**peak)
                      , start = list(amp = amp_init, phase = 0, per = init_value, peak=init_value/2)
                      , lower = list(amp = 0, phase=0, per = min_per, peak=init_value/4)
                      , upper = list(amp = Inf, phase=Inf, per = max_per, peak=Inf)
                      , algorithm = mode
                      , data = gd
      ...
      if (convergence_error1) {
        tryCatch({ # Fix period
          nls.model = nls(data ~ amp * (-1 + 2 * abs(cos(pi/init_value * time - phase*(pi/init_value)))**peak)
                          , start = list(amp = amp_init, phase = 0, peak = init_value/2)
                          , lower = list(amp=0, phase=0, peak = init_value/4)
                          , upper = list(amp=Inf, phase=Inf, peak = Inf)
                          , algorithm = mode
                          , data = gd
                          , control = list(maxiter = 200)
                          )
