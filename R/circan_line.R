#' circan_line
#'
#' Fit to a linear + sine curve in the frame of a nonlinear least squares model for accurate detection of circadian expression patterns.
...
#' @export
#' @examples
#' circan_line(data=expression_example, s2c=metadata_example, mode="port")
circan_line <- function(data, s2c, shiny = FALSE, mode = "default", init_value = 24,
  ...
  for (gene in 1:nrow(data)) {
    ...
    result = tryCatch({
      nls.model = nls(data ~ ((amp * cos((2*pi/per)*time + ((-2*pi/per)*phase)))+(m*time))
                      , start = list(amp = amp_init, phase = 0, per = init_value, m=0)
                      , algorithm = mode
                      , lower = list(amp=0, phase=0, per = min_per, m=0)
                      , upper = list(amp=Inf, phase=Inf, per = max_per, m=amp_init)
                      , data = gd
      ...
      if (convergence_error1) {
        tryCatch({ # Fix period
          nls.model = nls(data ~ (amp * cos((2*pi/init_value)*time + ((-2*pi/init_value)*phase)))+(m*time)
                        , start = list(amp = amp_init, phase = 0, m = 0)
                        , lower = list(amp=0, phase = 0, m = 0)
                        , upper = list(amp=Inf, phase = Inf, m = amp_init)
                        , algorithm = mode
                        , data = gd
                        , control = list(maxiter = 200)
                          )
