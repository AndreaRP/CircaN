#' circan_cosine2
#'
#' Fit to a squared cosine curve in the frame of a nonlinear least squares model for accurate detection of circadian expression patterns.
...
#' @export
#' @examples
#' circan_cosine2(data=expression_example, s2c=metadata_example, mode="port")
circan_cosine2 <- function(data, s2c, shiny = FALSE, mode = "default", init_value = 24,
  ...
  for (gene in 1:nrow(data)) {
    ...
    result = tryCatch({
      nls.model = nls(data ~ amp * cos(2*pi/per *(time - phase)) + (amp*0.50) * cos(2*pi/(per*0.3333)*(time -((phase + ((per*0.3333)*0.25)) %% per)))
                      ...
                      , data = gd, control = list(maxiter = 200))
      ...
      if (convergence_error1) {
        tryCatch({ # Fix period
          nls.model = nls(data ~ amp * cos(2*pi/init_value *(time - phase)) + (amp*0.50) * cos(2*pi/(init_value*0.3333)*(time -((phase + ((init_value*0.3333)*0.25)) %% init_value)))
