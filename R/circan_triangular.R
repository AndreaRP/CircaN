#' circan_triangular
#'
#' Fit to a triangular curve in the frame of a nonlinear least squares model for accurate detection of circadian expression patterns.
...
#' @export
#' @examples
#' circan_triangular(data=expression_example, s2c=metadata_example, mode="port")
circan_triangular <- function(data, s2c, shiny = FALSE, mode = "default", init_value = 24,
  ...
  for (gene in 1:nrow(data)) {
    ...
    result = tryCatch({
      nls.model <- nls(data ~ (amp*2/pi)*asin(cos((2*pi*time - 2*pi*phase)/per))
      ...
      if (convergence_error1) {
        tryCatch({ # Fix period
          nls.model = nls(data ~ (amp*2/pi)*asin(cos((2*pi*time - 2*pi*phase)/init_value))
